...
if (!storyId) {
  ...
  console.error("node prepare-story-insta-render.js <storyId>");
  console.error("");
  console.error("Esimerkki:");
  console.error("node prepare-story-insta-render.js 001");
  process.exit(1);
}

const templatePath = "templates/tarinakioski_template_insta_v3.aep";
const jobTemplatePath = "job-template-insta.json";
...
function readMetadata(filePath, expectedStoryId) {
  ...
  if (renderFormats.length > 0 && !renderFormats.includes("insta")) {
    console.warn(
      'Varoitus: metadata.json ei sisällä renderFormats-listassa arvoa "insta", mutta insta-jobi luodaan silti.',
    );
  }
...
function secondsToInclusiveFrameEnd(seconds) {
  // FrameStart on 0. Jos halutaan esimerkiksi 60 s 25 fps -renderi,
  // frameEnd on 1499 eli yhteensä 1500 frameä.
...
function createFinalVideoFileName(metadata, storyId) {
  ...
  return `${nameParts.join("_")}_insta.mp4`;
}
...
async function createRenderJob(metadata, backgroundPath, finalVideoPath) {
  ...
  if (videoWillBeTrimmed) {
    ...
    console.log("Videota ei lyhennetä. Koko video mahtuu mukaan.");
  }
...
async function main() {
  ...
  console.log("");
  console.log("Valittu versio: insta");
